  ...
  export default {
    ...
    data() {
      return {
        ...
        session:'',
        option :{
          color: ['#0f4787', '#f09932', '#2ef1af', '#f1532e', '#5f5bf9'],
          title: {
            text: '',
            show:false,
            left:0,
    ...
    methods: {
      ...
      refresh() {
        this.spinShow = true;
        this.$emit("success","session")  //传值到父类页面
      },
      childMethod(session){
        this.session = session.session[0].historical;
        this.select();
        this.spinShow = false;
      },
      select:function(){
        let dateTime = []; let data = [];
        if(this.session[this.timebox].values.length > 0){
          this.session[this.timebox].values.forEach(function(value,key){
